    ...
    public Result<Map<String, Object>> parcelInformation(String orderListId,
        ...
        if (isPlatform.intValue() == 0) {
            ...
            for (OrderProviderListDTO opl : orderProviderLists) {
                ...
                Map<String, Object> map = Maps.newHashMap();
                //字典数据
                logisticsCompany = iSysDictService.queryTableDictTextByKey("sys_dict_item", "item_text", "item_value", opl.getLogisticsCompany());

                map.put("logisticsCompany", logisticsCompany);
                map.put("trackingNumber", opl.getTrackingNumber());
                map.put("logisticsTracking", opl.getLogisticsTracking());
                if (CollUtil.isNotEmpty(opl.getOrderProviderGoodRecords())) {
                    map.put("mainPicture", opl.getOrderProviderGoodRecords().get(0).getMainPicture());
                }
                listMap.add(map);
            }
            //是否还有未发货商品
