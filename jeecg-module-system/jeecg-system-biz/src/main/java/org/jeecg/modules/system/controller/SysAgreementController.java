...
import org.jeecg.modules.system.entity.SysAgreement;
import org.jeecg.modules.system.service.ISysAgreementService;
...
public class SysAgreementController {
   ...
   public Result<IPage<SysAgreement>> queryPageList(SysAgreement sysAgreement,
       ...
       Result<IPage<SysAgreement>> result = new Result<IPage<SysAgreement>>();
   ...
   public Result<SysAgreement> add(@RequestBody SysAgreement sysAgreement) {
       Result<SysAgreement> result = new Result<SysAgreement>();
       try {
   ...
   public Result<SysAgreement> edit(@RequestBody SysAgreement sysAgreement) {
       Result<SysAgreement> result = new Result<SysAgreement>();
       SysAgreement sysAgreementEntity = sysAgreementService.getById(sysAgreement.getId());
       if(sysAgreementEntity==null) {
           result.error500("未找到对应实体");
       }else {
           boolean ok = sysAgreementService.updateById(sysAgreement);
           //TODO 返回false说明什么？
           if(ok) {
               result.success("修改成功!");
           }
   ...
   public Result<SysAgreement> deleteBatch(@RequestParam(name="ids",required=true) String ids) {
       Result<SysAgreement> result = new Result<SysAgreement>();
 ...
 public ModelAndView exportXls(HttpServletRequest request, HttpServletResponse response) {
     // Step.1 组装查询条件
     QueryWrapper<SysAgreement> queryWrapper = null;
     try {
         String paramsStr = request.getParameter("paramsStr");
         if (oConvertUtils.isNotEmpty(paramsStr)) {
             String deString = URLDecoder.decode(paramsStr, "UTF-8");
             SysAgreement sysAgreement = JSON.parseObject(deString, SysAgreement.class);
             queryWrapper = QueryGenerator.initQueryWrapper(sysAgreement, request.getParameterMap());
         }
     ...
     List<SysAgreement> pageList = sysAgreementService.list(queryWrapper);
     //导出文件名称
     mv.addObject(NormalExcelConstants.FILE_NAME, "协议管理列表");
     mv.addObject(NormalExcelConstants.CLASS, SysAgreement.class);
 ...
 public Result<?> importExcel(HttpServletRequest request, HttpServletResponse response) {
     ...
     for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
         ...
         try {
             List<SysAgreement> listSysAgreements = ExcelImportUtil.importExcel(file.getInputStream(), SysAgreement.class, params);
             sysAgreementService.saveBatch(listSysAgreements);
             return Result.ok("文件导入成功！数据行数:" + listSysAgreements.size());
         } catch (Exception e) {
             log.error(e.getMessage(),e);
